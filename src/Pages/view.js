...
class View extends React.Component {
  ...
  detail = (title, id) => {
    return this.props.extras.map((item) => {
      if (title === item.category) {
        ...
        return (
          <div key={item.id} className="extra-item mb-3 pt-3">
            <div className="ml-5 item-name">
            ...
            <div className="mt-2 btns mr-3">
              <Link to={`/edit-category/${item.id}`}>
                <button className="btns mr-2 pl-3 pr-3">Edit</button>
              </Link>
              <button
                onClick={() => {
                  this.props.deleteextra(item.id);
                }}
                className="btns">
                Remove
              </button>
    ...
    return (
      <div className="hide extra2">
        <div className="extra-tab">
          ...
          <div className="extras">{this.categories()}</div>
        </div>
      </div>
    );
  };
  view = () => {
    ...
    // }
    return this.props.view.map((item) => {
      return (
        <div key={item.id} className="main1 pb-2 pl-2 pr-2">
          ...
          <Link className="link" to={`/add-extra/${item.id}`}>
            <button id="register" className="e bg-success mt-2 mb-2">
              Add Extras
            </button>
          </Link>
          <button
...
const mapStateToProps = (state) => {
  return {
    view: state.view,
    categories: state.extracat,
    extras: state.allextras,
  };
};
